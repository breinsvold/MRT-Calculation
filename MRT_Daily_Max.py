...
# Stefan-Boltzmann constant for blackbody radiation calculations
SBC = 5.67e-8

# Surface properties for different environment types
# Albedo = reflectivity (urban is darker, absorbs more)
# Emissivity = how efficiently surface emits longwave radiation
ENV = {
    "urban": (0.12, 0.97),
    "rural": (0.25, 0.92),
    "mixed": (0.18, 0.95)
...
def calc_rad(tmax, tmin, hurs, lat, doy):
    """
    Calculates longwave and shortwave radiation components.
    Longwave uses Satterlund (1979) atmospheric emissivity method.
    Shortwave uses Bristow-Campbell (1984) temperature range method.
    """
    
    # --- LONGWAVE RADIATION ---
    # Calculate atmospheric emission based on temperature and humidity
    tas = (tmax + tmin) / 2
    tas_k = tas + 273.15
    ...
    # --- SHORTWAVE RADIATION ---
    # Calculate solar radiation based on latitude, day of year, and temp range
    ...
    # Diurnal temp range indicates cloud cover - larger range = clearer skies
    # Ensure dtr is non-negative (minimum 0.1 to avoid math errors)
...
def calc_mrt(tmax, tmin, hurs, lat, doy, alb, emis):
    """
    Calculates Mean Radiant Temperature by combining:
    - Sky longwave (atmospheric emission downward)
    - Ground longwave (surface emission upward)  
    - Absorbed shortwave (solar radiation not reflected)
    Returns MRT in degrees Celsius.
    """
...
def process_nc(model, scenario, period=None):
    """
    Main processing function - loads NetCDF climate data, calculates MRT
    for all grid cells and timesteps, outputs results to CSV.
    """
    if not HAS_XR:
        raise ImportError("xarray required")

    # Time index ranges for future scenario subsets
    ts = {
    ...
    # Select time period if specified (for future scenarios)
    ...
    # --- ESTIMATE TASMIN ---
    # Since we only have tas (daily mean) and tasmax, derive tasmin
    # Formula: tas = (tasmax + tasmin) / 2  =>  tasmin = 2*tas - tasmax
    # Clamp to ensure tasmin never exceeds tasmax (data inconsistencies)
    tmin = np.minimum(2 * tas - tmax, tmax - 0.1)
    print("  Estimating TASMIN from TAS and TASMAX")
    print("  Shape: " + str(tmax.shape))

    # --- PREPARE COORDINATE ARRAYS ---
    # Broadcast lat and day-of-year to match 3D data dimensions for vectorized calc
    ...
    # --- CALCULATE MRT FOR EACH ENVIRONMENT TYPE ---
    # Vectorized calculation across entire dataset (no loops needed here)
    ...
    # Loop through all grid cells and timesteps to build flat table
...
# Parses arguments and runs the processor
